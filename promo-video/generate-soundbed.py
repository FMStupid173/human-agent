...
def pluck(frequency: float, duration: float = 0.62) -> np.ndarray:
    ...
    return wave_body * np.exp(-t * 5.8) * 0.075
...
def add_pad(track: np.ndarray, start: float, duration: float, frequencies: tuple[float, ...]) -> None:
    ...
    pad *= envelope * 0.028 / len(frequencies)
...
def main() -> None:
    ...
    # D minor -> Bb major -> F major -> C major. Warm, reflective, and quietly decisive.
    progression = (
        ((146.83, 174.61, 220.00), 73.42),
    ...
    for bar_index in range(bars):
        ...
        for beat_index in range(4):
            ...
            add_mono(track, beat_start + BEAT / 2, hat(rng, beat_index == 3), 0.42)
