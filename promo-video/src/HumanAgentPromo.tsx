...
const FPS = 30;
const CUTS = [86, 226, 287, 481, 610, 737, 816];
...
const FrameChrome: React.FC = () => {
  ...
  return (
    <>
      ...
      <div style={{position: 'absolute', left: 56, right: 56, bottom: 42, display: 'flex', justifyContent: 'space-between', fontFamily, fontSize: 22, fontWeight: 900, color: chromeColor, zIndex: 60}}>
...
const captions = [
  ...
  [610, 737, '给结论前，再找能推翻自己的证据，跑完验证才收尾。'],
  [737, 816, '它不能保证 AI 永远正确，'],
  [816, 899, '但能让回答少一点套话，多一点判断和证据。'],
] as const;
      ...
      <div style={{position: 'absolute', left: 56, top: 74, right: 56}}>
        ...
        <div style={{marginTop: 46, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, opacity: c}}>
          {[
            ...
            <div key={top} style={{border: `3px solid ${C.ink}`, backgroundColor: index === 1 ? C.yellow : C.white, padding: '24px 20px', minHeight: 172}}>
      ...
      <div style={{position: 'absolute', left: 56, right: 56, top: 1405, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', border: `4px solid ${C.ink}`, backgroundColor: C.white}}>
        {[
          ...
          <div key={no} style={{padding: '24px 18px', minHeight: 135, borderLeft: i ? `3px solid ${C.ink}` : 'none', backgroundColor: i === 1 ? C.lime : C.white}}>
            <div style={{fontSize: 22, color: C.gray, fontWeight: 900}}>{no}</div>
...
const Bubble: React.FC<{side: 'user' | 'ai'; children: React.ReactNode; delay: number; bad?: boolean}> = ({side, children, delay, bad}) => {
  ...
  return (
    <div style={{alignSelf: side === 'user' ? 'flex-end' : 'flex-start', maxWidth: '88%', padding: '20px 24px', backgroundColor: side === 'user' ? C.blue : C.white, color: side === 'user' ? C.white : C.ink, border: `3px solid ${bad ? C.red : C.ink}`, fontSize: 30, lineHeight: 1.35, fontWeight: 700, opacity: a, transform: `translateY(${(1 - a) * 28}px)`}}>{children}</div>
  );
};
    ...
    <AbsoluteFill style={{fontFamily, backgroundColor: C.white, padding: '68px 56px 250px', boxSizing: 'border-box', overflow: 'hidden'}}>
      ...
      <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, height: 1280}}>
        <div style={{border: `4px solid ${C.ink}`, backgroundColor: C.paper, padding: 22, display: 'flex', flexDirection: 'column', gap: 18, position: 'relative'}}>
          <div style={{fontSize: 24, fontWeight: 900, color: C.gray}}>CHAT / 情绪边界</div>
        ...
        <div style={{border: `4px solid ${C.ink}`, backgroundColor: C.ink, color: C.white, padding: 22, display: 'flex', flexDirection: 'column', gap: 18, position: 'relative'}}>
          <div style={{fontSize: 24, fontWeight: 900, color: C.lime}}>DEBUG / 证据边界</div>
          <div style={{fontFamily: 'Consolas, monospace', backgroundColor: '#181818', border: '2px solid #525252', padding: 20, fontSize: 25, lineHeight: 1.55}}>
    ...
    <AbsoluteFill style={{fontFamily, backgroundColor: C.ink, color: C.white, overflow: 'hidden'}}>
      ...
      <div style={{position: 'absolute', left: 78, right: 78, top: 1060, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14}}>
        {[
          ...
          <div key={word} style={{backgroundColor: bg, color: bg === C.red || bg === C.cyan ? C.white : C.ink, padding: '20px 22px', fontSize: 28, fontWeight: 900, display: 'flex', justifyContent: 'space-between', opacity: rise(f, 10 + i * 4)}}>
    ...
    <AbsoluteFill style={{fontFamily, padding: '66px 56px 250px', boxSizing: 'border-box', overflow: 'hidden'}}>
      ...
      <div style={{position: 'relative'}}>
        ...
        <div style={{marginTop: 36, display: 'grid', gridTemplateColumns: '300px 1fr', gap: 22}}>
          <div style={{border: `4px solid ${C.ink}`, backgroundColor: C.white, padding: 22, minHeight: 600}}>
            <div style={{fontSize: 24, color: C.gray, fontWeight: 900}}>INPUT</div>
            ...
            <div style={{marginTop: 44, fontSize: 24, color: C.gray}}>当前消息优先<br />覆盖默认友好模板</div>
          </div>
          <div style={{border: `4px solid ${C.ink}`, backgroundColor: C.paper, padding: 22, minHeight: 600}}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><div style={{fontSize: 24, color: C.gray, fontWeight: 900}}>DYNAMIC HUMAN LAYER</div><div style={{width: 18, height: 18, backgroundColor: C.cyan, transform: `scale(${0.7 + selected * 0.5})`}} /></div>
          ...
          {[
            ...
            <React.Fragment key={title}>
              ...
              <div style={{border: `3px solid ${C.ink}`, backgroundColor: i === 1 ? C.lime : C.white, padding: '22px 16px', minHeight: 160}}>
                <div style={{fontSize: 21, color: C.gray, fontWeight: 900}}>{title}</div>
        ...
        <div style={{marginTop: 18, border: `4px solid ${C.ink}`, backgroundColor: C.ink, color: C.white, padding: '22px 24px'}}>
          <div style={{fontSize: 22, color: C.lime, fontWeight: 900}}>DEFAULT POLICY OVERRIDDEN</div>
    ...
    <AbsoluteFill style={{fontFamily, backgroundColor: C.ink, color: C.white, padding: '66px 56px 250px', boxSizing: 'border-box', overflow: 'hidden'}}>
      ...
      <div style={{marginTop: 54, display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 10}}>
        {['输入', '状态', '依赖', '输出', '不变量'].map((word, i) => {
          ...
          return <div key={word} style={{height: 210, backgroundColor: i === 4 ? C.lime : i === 2 ? C.red : '#202020', color: i === 4 ? C.ink : C.white, border: '2px solid #5A5A5A', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '20px 14px', boxSizing: 'border-box', opacity: a, transform: `translateY(${(1 - a) * 50}px)`}}><span style={{fontSize: 20, color: i === 4 ? C.ink : C.gray}}>0{i + 1}</span><strong style={{fontSize: 30}}>{word}</strong></div>;
        })}
      </div>
      ...
      <div style={{marginTop: 38, border: '3px solid #565656', backgroundColor: '#161616', padding: 24}}>
        <div style={{fontSize: 23, color: C.gray, fontWeight: 900, marginBottom: 20}}>EVIDENCE PATH / 用一条证据切开边界</div>
        {[
          ...
          <div key={no} style={{display: 'grid', gridTemplateColumns: '58px 190px 1fr', alignItems: 'center', gap: 14, borderTop: '2px solid #3E3E3E', padding: '18px 0'}}>
            <span style={{color, fontSize: 22, fontWeight: 900}}>{no}</span>
            <strong style={{fontSize: 26}}>{title}</strong>
            <span style={{fontSize: 23, color: '#C8C8C8'}}>{desc}</span>
          </div>
        ))}
      </div>
      <div style={{marginTop: 28, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12}}>
        {[
          ...
          <div key={title} style={{border: `3px solid ${color}`, minHeight: 160, padding: '20px 16px'}}>
            ...
            <div style={{fontSize: 23, lineHeight: 1.3, marginTop: 18}}>{desc}</div>
          </div>
        ))}
      </div>
    </AbsoluteFill>
  ...
  return (
    <AbsoluteFill style={{fontFamily, backgroundColor: C.yellow, padding: '66px 56px 250px', boxSizing: 'border-box', overflow: 'hidden'}}>
      ...
      <div style={{marginTop: 42, display: 'grid', gridTemplateColumns: '1fr 90px 1fr', gap: 14, alignItems: 'stretch'}}>
        <div style={{border: `4px solid ${C.ink}`, backgroundColor: C.white, padding: 24}}>
          <div style={{fontSize: 23, color: C.gray, fontWeight: 900}}>CURRENT HYPOTHESIS</div>
        ...
        <div style={{border: `4px solid ${C.ink}`, backgroundColor: C.ink, color: C.white, padding: 24}}>
          <div style={{fontSize: 23, color: C.lime, fontWeight: 900}}>FALSIFYING EVIDENCE</div>
          <div style={{fontSize: 31, lineHeight: 1.35, fontWeight: 800, marginTop: 30}}>接口返回新数据，页面仍显示旧值。</div>
          <div style={{fontSize: 25, color: C.yellow, marginTop: 28}}>→ 先查前端状态与竞态</div>
        </div>
      </div>
      ...
      <div style={{marginTop: 30, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12}}>
        {[
          ...
          <div key={title} style={{border: `4px solid ${C.ink}`, backgroundColor: i === 1 ? C.ink : C.white, color: i === 1 ? C.white : C.ink, minHeight: 190, padding: '22px 16px'}}>
            ...
            <div style={{fontSize: 22, lineHeight: 1.35, fontWeight: 800, marginTop: 18}}>{desc}</div>
          </div>
        ))}
      </div>
      <div style={{marginTop: 26, border: `4px solid ${C.ink}`, backgroundColor: C.white, padding: '18px 20px', display: 'grid', gridTemplateColumns: '140px 1fr 180px', alignItems: 'center', gap: 18}}>
        <span style={{fontSize: 23, color: C.gray, fontWeight: 900}}>DONE GATE</span>
        <span style={{fontSize: 27, fontWeight: 900}}>修复 + 测试 + 边界检查</span>
        <span style={{fontSize: 25, color: C.cyan, fontWeight: 900}}>全部通过</span>
      </div>
...
const Reliability: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{fontFamily, padding: '66px 56px 250px', boxSizing: 'border-box', backgroundColor: C.white, overflow: 'hidden'}}>
      <Label>06 / 可靠性边界</Label>
      <div style={{fontSize: 68, lineHeight: 1.08, fontWeight: 900}}>它不会保证<br />AI 永远正确。</div>
      <div style={{marginTop: 34, backgroundColor: C.red, color: C.white, display: 'inline-block', padding: '14px 20px', fontSize: 31, fontWeight: 900}}>拒绝“消灭幻觉”的假承诺</div>
      <div style={{marginTop: 42, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14}}>
        ...
        ].map(([title, desc, bg, color], i) => <div key={title} style={{backgroundColor: bg, color, minHeight: 190, padding: 20, opacity: rise(f, i * 8), transform: `translateY(${(1 - rise(f, i * 8)) * 30}px)`}}><div style={{fontSize: 31, fontWeight: 900}}>{title}</div><div style={{fontSize: 23, lineHeight: 1.35, marginTop: 18}}>{desc}</div></div>)}
      </div>
      <div style={{position: 'absolute', left: 56, right: 56, top: 890, bottom: 270, border: `4px solid ${C.ink}`, display: 'grid', gridTemplateRows: 'repeat(4,1fr)'}}>
        {[
          ...
          <div key={kind} style={{display: 'grid', gridTemplateColumns: '160px 1fr 250px', alignItems: 'center', gap: 16, padding: '0 22px', borderTop: i ? `3px solid ${C.ink}` : 'none', backgroundColor: i % 2 ? C.paper : C.white}}>
            ...
            <span style={{fontSize: 23, color: C.gray}}>{scope}</span>
            <span style={{fontSize: 23, fontWeight: 900, color: i === 3 ? C.red : C.cyan}}>{action}</span>
          </div>
        ))}
      </div>
    </AbsoluteFill>
  ...
  return (
    <AbsoluteFill style={{fontFamily, backgroundColor: C.red, color: C.white, padding: '66px 56px 250px', boxSizing: 'border-box', overflow: 'hidden'}}>
      ...
      <div style={{marginTop: 54, display: 'flex', flexWrap: 'wrap', gap: 10}}>
        {['Codex', 'Claude Code', 'Gemini', 'Kimi', 'ChatGPT adapter'].map((word, i) => <span key={word} style={{backgroundColor: i === 4 ? C.ink : C.white, color: i === 4 ? C.white : C.ink, padding: '12px 18px', fontSize: 24, fontWeight: 900}}>{word}</span>)}
      </div>
      <div style={{position: 'absolute', left: 56, right: 56, top: 865, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12}}>
        {[
          ...
          <div key={title} style={{minHeight: 220, border: `4px solid ${C.ink}`, backgroundColor: i === 1 ? C.lime : C.white, color: C.ink, padding: '24px 18px'}}>
            <div style={{fontSize: 56, fontWeight: 900, lineHeight: 1}}>0{i + 1}</div>
            <div style={{fontSize: 28, fontWeight: 900, marginTop: 24}}>{title}</div>
            <div style={{fontSize: 21, fontWeight: 800, marginTop: 12}}>{desc}</div>
          </div>
        ))}
      </div>
      <div style={{position: 'absolute', left: 56, right: 56, top: 1260, backgroundColor: C.white, color: C.ink, border: `5px solid ${C.ink}`, padding: '26px 24px'}}>
...
export const HumanAgentPromo: React.FC = () => (
  <AbsoluteFill style={{backgroundColor: C.paper}}>
    ...
    <Audio src={staticFile('soundbed.mp3')} volume={0.16} />
    ...
    <Sequence from={610} durationInFrames={127}><AdversarialFinish /></Sequence>
    <Sequence from={737} durationInFrames={79}><Reliability /></Sequence>
